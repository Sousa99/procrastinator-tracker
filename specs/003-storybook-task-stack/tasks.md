...
**Goal**: A presentational `TaskDeck` component renders tasks as a **swipeable card stack** (Deck Standard 1 style — top card fully visible, next `stackSize - 1` cards scaled/fanned behind it), with `filters` for which tasks are shown, an `autoRotateMs` timer (default 4000, `0` off), `loop` (default true), and drag-to-skip that resets the timer. The self-fetching `TaskDeckWrapper` owns retrieval; both are demonstrable in Storybook.
...
- [X] T014 [P] [US1] Create `frontend/src/components/task/TaskDeck.tsx` — presentational swipeable deck per `data-model.md` (`tasks`, `filters`, `autoRotateMs`, `loop`, `stackSize`, `renderCard?`, `onCardChange?`, `className?`), router-free full-card default, urgency-descending order with unset last, client-side filter matching. Stage is `w-full h-[24rem] sm:h-[26rem]`; cards uniform (`h-full flex flex-col overflow-hidden`) with `line-clamp-2` ellipsis on long text.
