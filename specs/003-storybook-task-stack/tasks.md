...
- [X] T014 [P] [US1] Create `frontend/src/components/task/TaskDeck.tsx` — presentational swipeable deck per `data-model.md` (`tasks`, `filters`, `autoRotateMs`, `loop`, `stackSize`, `slideDurationMs`, `renderCard?`, `onCardChange?`, `className?`), router-free full-card default, urgency-descending order with unset last, client-side filter matching. Stage is `w-full h-[24rem] sm:h-[26rem]`; cards uniform (`h-full flex flex-col overflow-hidden`) with content anchored (description top, meta/tags/assignees bottom via `mt-auto`) and `line-clamp-2` ellipsis on long text.
