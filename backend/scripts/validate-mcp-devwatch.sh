...
echo "=== T010: dev:mcp auto-reload ==="
kill_port 3001
pnpm dev:mcp >/tmp/mcp-devwatch.log 2>&1 &
MCP_PID=$!
for i in $(seq 1 10); do sleep 1; curl -s -o /dev/null http://localhost:3001/health && break; done
echo "before reload:"; mcp_call '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"task.list","arguments":{}}}' | head -c 80; echo

# Trigger a reload by touching a watched source file
touch src/services/tasks.ts
sleep 4

echo "listening lines in log: $(grep -c 'MCP server listening' /tmp/mcp-devwatch.log)"
echo "after reload:"; mcp_call '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"task.list","arguments":{}}}' | head -c 80; echo
